...
/// A nursery address as a major-heap field stores it.
///
/// The major-heap proofs below need exactly one property of such a value:
/// that it can never be mistaken for a major-heap pointer.  That follows from
/// `GC.Spec.ZeroAddr.zero_addr_above_minor_size`, which places the whole
/// nursery strictly below `zero_addr`.
///
/// Nothing here says the address is an *object* address.  An address interior
/// to a nursery closure --- an OCaml infix pointer --- satisfies the same
/// refinement, which is what lets one construction serve both
/// `GC.SPOT.ConcreteMajor` (a plain nursery pointer) and
/// `GC.SPOT.ConcreteMajorInfix` (an interior one).
let minor_ptr = v:U64.t{U64.v v >= 8 /\ U64.v v % 8 == 0 /\ U64.v v < 2048}
...
val spot_major_heap : (r:unit{spot_major_room}) -> (mp:minor_ptr) -> Tot heap
...
val spot_major_c_reads
  : r:unit{spot_major_room} -> mp:minor_ptr ->
    Lemma (ensures (
      ...
      SpecHeap.read_word major (spot_c_field1 r) == mp /\
...
val spot_major_objects
  : r:unit{spot_major_room} -> mp:minor_ptr ->
    Lemma (ensures
      SpecFields.objects zero_addr (spot_major_heap r mp) ==
...
val spot_major_object_cases
  : r:unit{spot_major_room} -> mp:minor_ptr -> obj:obj_addr ->
    Lemma (requires FStar.Seq.mem obj (SpecFields.objects zero_addr (spot_major_heap r mp)))
          (ensures obj == spot_c r \/ obj == spot_free_obj r)

val spot_major_free_field_read
  : r:unit{spot_major_room} -> mp:minor_ptr -> j:nat ->
    Lemma (requires j < spot_free_wosize r /\
                    U64.v (spot_free_obj r) + j * 8 + 8 <= heap_size)
          (ensures
            SpecHeap.read_word (spot_major_heap r mp)
              (U64.uint_to_t (U64.v (spot_free_obj r) + j * 8)) == 0UL)

val spot_major_heap_shape
  : r:unit{spot_major_room} -> mp:minor_ptr ->
    Lemma (ensures GenInv.major_heap_shape (spot_major_heap r mp) (spot_major_fp r))

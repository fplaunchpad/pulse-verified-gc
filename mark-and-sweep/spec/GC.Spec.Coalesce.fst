...
// The merge target (first_blue) is always <= the address of anything
/// currently inside the run's address range. Since coalesce always merges
/// into the leftmost object of a run, this is the fact that makes pointer
/// redirection during merging preserve (or improve) decreasing chains.
val first_blue_is_run_min
  (first_blue: U64.t) (run_words: pos) (addr: obj_addr)
  : Lemma
    (requires
      U64.v first_blue >= U64.v mword /\
      U64.v first_blue % U64.v mword == 0 /\
      U64.v addr >= U64.v first_blue /\
      U64.v addr < U64.v first_blue - U64.v mword + run_words * U64.v mword)
    (ensures U64.v first_blue <= U64.v addr)

let first_blue_is_run_min first_blue run_words addr = ()
...
/// Walk from any position s at or before the merged header: every object
/// found satisfies the trichotomy. Recursion steps forward one object at
/// a time until it reaches the merged header itself.
val flush_walk_trichotomy_from
  (g: heap) (first_blue: obj_addr) (run_words: nat) (fp: U64.t)
  (start: hp_addr) (s: hp_addr) (x: obj_addr)
  : Lemma
    (requires
      Seq.length g == heap_size /\
      run_words >= 2 /\
      ...
      (forall (t: hp_addr).
        ...
        U64.v t + U64.v mword < heap_size ==>
        (let w = getWosize (read_word (fst (flush_blue g first_blue run_words fp)) t) in
         U64.v t + (U64.v w + 1) * U64.v mword <= U64.v first_blue - U64.v mword)) /\
      (let g' = fst (flush_blue g first_blue run_words fp) in
       Seq.mem x (objects s g')))
    (ensures
      x = first_blue \/
      (let g' = fst (flush_blue g first_blue run_words fp) in
       U64.v (hd_address x) + (U64.v (wosize_of_object x g') + 1) * U64.v mword
         <= U64.v first_blue - U64.v mword) \/
      U64.v (hd_address x) >=
        U64.v first_blue - U64.v mword + run_words * U64.v mword)
    (decreases (U64.v first_blue - U64.v mword - U64.v s))

/// wosize read via the object address equals wosize read via its header address
let wosize_at_f_address (g: heap) (s: hp_addr{U64.v s + U64.v mword < heap_size})
  : Lemma (U64.v (wosize_of_object (f_address s) g) == U64.v (getWosize (read_word g s)))
  = f_address_spec s;
    hd_address_spec (f_address s);
    wosize_of_object_spec (f_address s) g;
    assert (hd_address (f_address s) == s)

let rec flush_walk_trichotomy_from g first_blue run_words fp start s x =
  hd_address_spec first_blue;
  hd_address_spec x;
  let g' = fst (flush_blue g first_blue run_words fp) in
  flush_blue_preserves_length g first_blue run_words fp;
  if U64.v s = U64.v first_blue - U64.v mword then begin
    flush_blue_header_spec g first_blue run_words fp;
    merged_block_decompose g' first_blue run_words start x;
    if x = first_blue then ()
    else begin
    ...
    let hdr = read_word g' s in
    let wz = getWosize hdr in
    let next_nat = U64.v s + (U64.v wz + 1) * U64.v mword in
    objects_nonempty_next s g';
    f_address_spec s;
    let obj = f_address s in
    mem_cons_lemma x obj (Seq.tail (objects s g'));
    if x = obj then begin
      hd_address_spec obj;
      f_address_spec s;
      wosize_at_f_address g' s;
      let w = wosize_of_object obj g' in
      assert (U64.v w == U64.v wz);
      assert (U64.v (hd_address obj) == U64.v s);
      assert (U64.v s < U64.v first_blue - U64.v mword);
      assert (U64.v s + U64.v mword < heap_size);
      assert (U64.v s + (U64.v w + 1) * U64.v mword <= U64.v first_blue - U64.v mword)
    end
    else begin
      if next_nat >= heap_size then begin
        objects_tail_empty_when_done s g';
        assert (Seq.tail (objects s g') == Seq.empty)
      end
      else begin
        ...
        assert (U64.v next > U64.v s);
        flush_walk_trichotomy_from g first_blue run_words fp start next x
      end
    end
  end

/// Dichotomy over the full heap walk after a flush: every object either
/// lies entirely before the flushed run, is the merged block itself, or
/// lies entirely after the run. Absorbed objects do not appear.
val flush_walk_trichotomy
  (g: heap) (first_blue: obj_addr) (run_words: nat) (fp: U64.t)
  (start: hp_addr) (x: obj_addr)
  : Lemma
    (requires
      Seq.length g == heap_size /\
      run_words >= 2 /\
...
let flush_walk_trichotomy g first_blue run_words fp start x =
  flush_walk_trichotomy_from g first_blue run_words fp start zero_addr x

val coalesce_aux_chain_decreasing
  (g0 g: heap) (start: hp_addr) (objs all_objs: seq obj_addr)
  (first_blue: U64.t) (run_words: nat) (fp: U64.t)
  : Lemma
    (requires
      walk_pre g0 g start objs all_objs first_blue run_words /\
      objects zero_addr g == all_objs /\
      (forall (y: obj_addr).
        Seq.mem y (objects zero_addr (fst (flush_blue g first_blue run_words fp))) ==>
        U64.v (wosize_of_object y (fst (flush_blue g first_blue run_words fp))) >= 1) /\
      (forall (addr: hp_addr). U64.v addr >= U64.v start ==>
        read_word g addr == read_word g0 addr) /\
      (fp = 0UL \/
       (is_pointer_field fp /\
      ...
      (run_words > 0 ==> U64.v zero_addr <= U64.v first_blue - U64.v mword) /\
      (run_words > 0 ==>
       (forall (t: hp_addr).
        U64.v t < U64.v first_blue - U64.v mword /\
        U64.v t % U64.v mword == 0 /\
        ...
        (let w = getWosize (read_word (fst (flush_blue g first_blue run_words fp)) t) in
         U64.v t + (U64.v w + 1) * U64.v mword <= U64.v first_blue - U64.v mword))) /\
      (forall (x: obj_addr).
        Seq.mem x all_objs /\ ~(Seq.mem x objs) /\ is_blue x g ==>
...
#push-options "--z3rlimit 400 --fuel 1 --ifuel 1 --split_queries always"
let rec coalesce_aux_chain_decreasing g0 g start objs all_objs first_blue run_words fp =
  if Seq.length objs = 0 then begin
    ...
    if run_words = 0 then begin
          ...
          (ensures
            (let v = read_word g x in
             ...
             (is_pointer_field v /\ U64.v v < U64.v x /\
              Seq.mem (v <: obj_addr) (objects zero_addr g) /\
              is_blue (v <: obj_addr) g)))
        = assert (Seq.mem x all_objs)
      in
      FStar.Classical.forall_intro (FStar.Classical.move_requires aux0)
    end
      ...
      let aux (x: obj_addr)
        ...
        = if x = (first_blue <: obj_addr) then begin
            ...
            flush_walk_trichotomy g (first_blue <: obj_addr) run_words fp start x;
            hd_address_spec x;
            assert (U64.v (wosize_of_object x g') >= 1);
            assert (U64.v (hd_address x) == U64.v x - U64.v mword);
            flush_blue_preserves_outside g first_blue run_words fp (hd_address x);
            flush_blue_preserves_outside g first_blue run_words fp x;
            admit ()
          end
      in
      FStar.Classical.forall_intro (FStar.Classical.move_requires aux)

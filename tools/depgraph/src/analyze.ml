...
let rec walk_dir_seen ~(recursive : bool) (seen : (string, unit) Hashtbl.t)
                      (dir : string) (f : string -> unit) : unit =
  (* Guard against symlink loops: only descend into each physical directory
     once. *)
  let key = match Unix.realpath dir with exception _ -> dir | r -> r in
  if Hashtbl.mem seen key then ()
    ...
    | entries ->
      Array.iter (fun e ->
        let p = dir ^/ e in
        (* Sys.is_directory raises Sys_error on dangling symlinks.  Skip any
           entry we cannot stat rather than aborting the entire scan. *)
        match Sys.is_directory p with
        | exception _ -> ()
        | true -> if recursive then walk_dir_seen ~recursive seen p f
        | false -> f p) entries
  end

let walk_dir ~(recursive : bool) (dir : string) (f : string -> unit) : unit =
  walk_dir_seen ~recursive (Hashtbl.create 64) dir f
